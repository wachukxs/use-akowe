...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const keywordPage = await getKeywordPageBySlugAndCategory(slug, 'template');
...
export default async function TemplateKeywordPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbData = getBreadcrumbStructuredData(
    [
      ...
      { label: title, href: `/templates-keywords/${slug}` },
    ],
    baseUrl
  );

  const templateSlugs = await getAllKeywordSlugs('template');
  const templatePages = await Promise.all(templateSlugs.map((s) => getKeywordPageBySlugAndCategory(s, 'template')));
    ...
    <div className="min-h-screen bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
      ...
      <main className="max-w-4xl mx-auto px-6 sm:px-8 lg:px-12 py-12">
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(generateWebPageSchema({
              url: `${baseUrl}/en/templates-keywords/${slug}`,
              title: title,
              description: keywordPage.description,
            })),
          }}
        ...
        <article className="prose prose-lg max-w-none">
          ...
          {allTemplateKeywords.length > 0 && (
            <div className="mt-12">
              <RelatedContent
                title="Related Templates"
                items={allTemplateKeywords.map((k) => ({
                  title: k.title,
                  description: k.description,
                  href: `/templates-keywords/${k.slug}`,
                }))}
              />
            </div>
          )}
          ...
          <div className="border-4 border-[hsl(var(--border-strong))] bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] p-6 rounded-lg mt-8">
            ...
            <p className="mb-6 text-lg leading-relaxed">
              Use Akowe to apply these templates in your own research. Get AI-powered writing assistance, 
