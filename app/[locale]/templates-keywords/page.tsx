  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
      ...
      <main className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-12">
        <Breadcrumbs
          items={[{ label: 'Templates', href: '/templates-keywords' }] as Array<{ label: string; href: string }>}
        />
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(getBreadcrumbStructuredData(
              [{ label: 'Templates', href: '/templates-keywords' }],
              baseUrl
            )),
          }}
        />
        <div className="mb-12">
          ...
          <p className="text-xl text-[hsl(var(--muted-foreground))]">
            Start with a structured foundation. Browse templates for research papers, theses, lab reports, and other academic documents.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pages.map((page) => (
            <Link
              key={page.slug}
              href={`/templates-keywords/${page.slug}`}
