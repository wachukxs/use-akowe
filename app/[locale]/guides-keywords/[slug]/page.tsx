...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const keywordPage = await getKeywordPageBySlugAndCategory(slug, 'guide');
...
export default async function GuideKeywordPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const guidePages = await Promise.all(guideSlugs.map((s) => getKeywordPageBySlugAndCategory(s, 'guide')));
  const allGuideKeywords = guidePages
    ...
    <div className="min-h-screen bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
      ...
      <main className="max-w-4xl mx-auto px-6 sm:px-8 lg:px-12 py-12">
        ...
        <article className="prose prose-lg max-w-none">
          ...
          {allGuideKeywords.length > 0 && (
            <div className="mt-12">
              <RelatedContent
                title="Related Guides"
                items={allGuideKeywords.map((k) => ({
                  title: k.title,
                  description: k.description,
                  href: `/guides-keywords/${k.slug}`,
                }))}
              />
            </div>
          )}
