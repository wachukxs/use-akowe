...
export default function AdminDashboard() {
  ...
  const runBackfillActivation = async () => {
    ...
    try {
      ...
      setBackfillStatus({
        running: false,
        result: `Done: ${data.created} created, ${data.updated} updated, ${data.newlyActivated} newly activated (${data.usersWithProjects} users with projects, ${data.alreadyTracked} already tracked)`,
      });
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--background))] p-6">
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        {/* Backfill First Project (temporary) */}
        <div className="flex items-center gap-3 p-3 border-2 border-dashed border-[hsl(var(--warning))] bg-[hsl(var(--surface))] rounded-lg">
          ...
          >
            {backfillStatus.running ? 'Running...' : 'Backfill First Project'}
          </button>
          <span className="text-xs text-[hsl(var(--muted-foreground))]">
            {backfillStatus.result || 'Retroactively set firstProjectCreatedAt for users who created projects before tracking was added.'}
          </span>
        </div>
